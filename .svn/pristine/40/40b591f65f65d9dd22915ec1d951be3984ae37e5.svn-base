...
$scope.knowYourWard = function () {
 ...
 // });
  ...
  var marker;
  var wardIcon = L.icon({
        iconUrl: 'img/mapIcon/wardIcon.png',
        iconSize: [30, 30]
    });

  marker = L.marker([$scope.lat, $scope.long]).addTo($scope.map).bindPopup('My Location').openPopup();
...
//No longer in Use, for Defect. #208213
      ...
      for (var i = 0; i < geocodeResponse.length; i++) {
       //if (e.latlng) {
        ...
        //   console.log(geocodeResponse[i].name, "00000000")
          ...
          markers = L.marker([geocodeResponse[i].latitude, geocodeResponse[i].longitude],{icon: wardIcon}).bindPopup(geocodeResponse[i].ward_name).openPopup().addTo($scope.map);
  ...
  RestService.getYourWardZoneByLocNew($scope.lat,$scope.long).then(function (geocodeResponse) {
    console.log("know your ward zone geocodeResponse -->" + JSON.stringify(geocodeResponse));
    if (geocodeResponse.length > 0) {
      $ionicLoading.hide();
  ...
  function (err) {
    ...
    console.log("Error in Know Your",err);
    toaster.error($filter('translate')('KNOWYOURTHIRDPARTYERROR'));
    $ionicLoading.hide();
  });
