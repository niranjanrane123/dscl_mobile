angular.module('starter').controller('EstateBookingDetailsCtrl', function ($scope, RestService, $ionicLoading, $stateParams,
toaster, $filter, $state, $sessionStorage,$localStorage,$ionicPopup,$rootScope) {
	...
	$scope.bookNow = function()
	{
		...
		.then(function(response){
			...
			console.log("disabledDates response: "+JSON.stringify(disabledDates));
			$state.go("app.estateBookPropDetail",{response:JSON.stringify(response), "disabledDates":JSON.stringify(disabledDates)});
...
//	$scope.orgId = $localStorage.responselogindata.orgId;
  ...
  $scope.openLink = function(){
	...
	var _init = function (){
      $('.ui-datepicker').hide();
	    $('.hasDatepicker').blur();
	    RestService.getTermsAndCondition($stateParams.propId,$scope.orgId)
      	.then(function(response){
      	  console.log("termsAndConditionResponse: "+JSON.stringify(response));
      	  if(response){
           $scope.url = "http://192.168.100.157:8090/"+response.slice(0,2)+'/'+response.slice(4,15)+'/'+response.slice(17,30)+'/'+response.slice(32,33)+'/'+response.slice(35);
      	   $localStorage.termsAndConditionURL = $scope.url;
      	  }
      });
	};
	_init();
