angular.module('starter')
    .controller('EnroutedDetailsCTRL', function ($scope, RestService, $ionicLoading, $stateParams, toaster, $filter, ENV, $ionicHistory,
      ...
      $scope.AccessToken = function(type){
        console.log('$rootScope.swmAccessToken' + $rootScope.swmAccessToken, type)
          ...
          RestService.getAccessToken().then(function (data) {
            ...
            if(type == 'enroute'){
              ...
              $scope.getEnroutedDetails($rootScope.swmAccessTokenData);
            }
          },
            function (err) {
        ...
        $scope.getEnroutedDetails= function(token){
            var params = {
                "sourceId": 0,
                "destinationId": 0,
            ...
            RestService.enroutedDetails(params,token).then(function (resp) {
                ...
                function (err) {
                    ...
                    console.log("Error in Bus Route enroute",err)
        ...
        $scope.view = {
            ...
            init: function () {
                if($localStorage.langID == "2"){
                  ...
                  $scope.getEnroutedDetails(token);
