...
.controller('EstateBookingCtrl', function ($scope, RestService, $ionicLoading, $stateParams, toaster, $filter, ENV, $state,
 $localStorage, $rootScope) {
//	$scope.orgId = $localStorage.responselogindata.orgId;
	$scope.orgId = $localStorage.selectedorgID;
	$scope.level = "2";
	$scope.lookUpCode = "ETY";
	$scope.lookUpCode1 = "EVT";
	disabledDates = [];
	$scope.capacityFrom;
	$scope.capacityTo;
	$scope.rentFrom;
  ...
  $scope.initializeEvent = function(event){
    $scope.event=event
  }
	$scope.search = function(){
	$ionicLoading.show();
	console.log("bvbn", $scope.category);
	if($scope.rentTo == undefined){
	 $scope.startRent=99999999
	}else{
	$scope.startRent = $scope.rentTo;
	}
	if($scope.rentFrom == undefined)
	{
   $scope.endRent=0
  }else{
   $scope.endRent = $scope.rentFrom;
  }
  if($scope.capacityFrom == undefined)
  {
   $scope.endCapacity=0
  }else{
   $scope.endCapacity = $scope.capacityFrom;
  }
  if($scope.capacityTo == undefined)
  {
     $scope.startCapacity=99999999
    }else{
     $scope.startCapacity = $scope.capacityTo;
    }
	var RNLParams = {
          "orgId": $scope.orgId,
          "categoryTypeId": $scope.category,
          "rentFrom": $scope.endRent,
          "rentTo":  $scope.startRent,
          "eventId": $scope.event,
          "capcityFrom": $scope.endCapacity,
          "capcityTo": $scope.startCapacity

  }
  console.log("rnlparam-->",JSON.stringify(RNLParams));
	RestService.getFilterList(RNLParams)
  		.then(function(response){
  		console.log('response tulika',JSON.stringify(response.estatePropResponseDTOs.length))
      if(response.estatePropResponseDTOs.length > 0){
            $scope.showPropDetail = true;
            $scope.propertyListResponse = response;
            console.log("getRNLPropFilterList: "+JSON.stringify($scope.propertyListResponse));
      }
      else {
      $scope.propertyListResponse=[];
        toaster.error($filter('translate')('NO_RECORD'));
      }
  			$ionicLoading.hide();
  		},function(propertyerr){
...
//				alert("calendarDataResponse: "+JSON.stringify($scope.calendarDataResponse));
				$scope.calEventsExt = {
					color: '#eb5055',
					textColor: '#ffffff',
					events: []
					...
					if(shiftName == "General"){
						...
						if(dateSearch.length == 3){
							if(disabledDates.indexOf(shiftDate) == -1) {
								disabledDates.push(convert(new Date(shiftDate)));
							}
						}
					}
				}
    ...
    $scope.openAmenityDetail = function(propId){
      $localStorage.propId =propId;
      $state.go("app.estateAmenityDetail");
    }
    var _init = function (){
    	RestService.getNHPrefixData($scope.lookUpCode1 ,$scope.orgId)
      	.then(function(response){
      		if(response.length > 0){
      			$scope.eventResponse = response;
      			console.log("eventResponse: "+JSON.stringify($scope.eventResponse));
      			$scope.eventList = new Array();
      			for(var i=0;i<$scope.eventResponse.length;i++){
      				$scope.eventList.push({
      					eventValue : $scope.eventResponse[i].lookUpId,
      					eventName : $scope.eventResponse[i].descLangFirst
      				})
      			}
      		}
      		else {
      		...
      		console.log("$scope.categoryList: "+JSON.stringify($scope.eventList));
      	},function(categoryerr){
      		toaster.error($filter('translate')('ERROR'), $filter('translate')('ERROR_OCCURED'));
      		$state.go("app.home");
      	})
