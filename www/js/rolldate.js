 ...
 */
 ...
 ! function(t, i) {
     ...
     function i() {
         ...
         if (t.value) {
             //t.el && ("input" == e.nodeName.toLowerCase() ? e.value = t.value : e.innerText = t.value); //set default value
             var o = t.value.replace(/-/g, "/").replace(/[^\d/:\s]/g, ""),
                 s = new Date(o);
             s && "Invalid Date" != s ? t.el ? e.bindDate = s : i.bindDate = s : console.error("Invalid Dates" + o)
         }
     }
     return i.prototype = {
         constructor: i,
             ...
             for (var n = this, t = n.baseData(), r = n.config, a = t.domId, l = r.format.split(/-|\/|\s|:/g), i = l.length, e = "", h = r.el ? S(r.el).bindDate || new Date : n.bindDate || new Date, c = r.lang, o = 0; o < i; o++) {
                 ...
                 if (e += '<div id="' + a[s] + '"><ul class="wheel-scroll">', "YYYY" == s)
                     ...
                     for (var u = 1; u <= 12; u++) {
                         ...
                         (u == 11) ? PP = 'November' : '';
                         (u == 12) ? PP = 'December' : '';
                         e += '<li class="wheel-item ' + (u == h.getMonth() + 1 ? "active" : "") + '" data-index="' + d + '">' + (u < 10 ? PP : PP) + "</li>", d++;
                     }
                 }
                 else if ("DD" == s) {
                     for (var m = n.getMonthlyDay(h.getFullYear(), h.getMonth() + 1), f = 1; f <= m; f++) e += '<li class="wheel-item ' + (f == h.getDate() ? "active" : "") + '" data-index="' + d + '">' + (f < 10 ? "0" + f : f) + "</li>", d++;
                 }
                 else if ("hh" == s)
                     for (var v = 0; v <= 23; v++) e += '<li class="wheel-item ' + (v == h.getHours() ? "active" : "") + '" data-index="' + d + '">' + (v < 10 ? "0" + v : v) + "</li>", d++;
                 else if ("mm" == s)
                     for (var g = 0; g <= 59; g += r.minStep) e += '<li class="wheel-item ' + (g == h.getMinutes() ? "active" : "") + '" data-index="' + d + '">' + (g < 10 ? "0" + g : g) + "</li>", d++;
                 else if ("ss" == s)
                     for (var w = 0; w <= 59; w++) e += '<li class="wheel-item ' + (w == h.getSeconds() ? "active" : "") + '" data-index="' + d + '">' + (w < 10 ? "0" + w : w) + "</li>", d++;
                 e += "</ul></div>"
             }
         ...
         event: function() {
             ...
             a.tap(t, function() {
                 ...
                 for (var s in a.scroll) {
                     ...
                     if (!n) {
                         var sumMonth = monthSelected + 1;
                         (sumMonth < 10) ? sumMonth = "0" + sumMonth : sumMonth;
                         n = sumMonth.toString();
                     }
         ...
         getSelected: function(t) {
             if (t.wrapper.id == 'rolldate-month') {
                 monthSelected = t.getSelectedIndex();
             }
